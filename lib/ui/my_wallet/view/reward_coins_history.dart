...
import 'package:drama_chaska/custom_widget/no_data_widget.dart';
import 'package:drama_chaska/main.dart';
import 'package:drama_chaska/shimmer/transaction_history_shimmer.dart';
...
class RewardCoinsHistory extends StatelessWidget {
  const RewardCoinsHistory({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          EnumLocal.rewardCoinsHistory.name.tr,
              ...
              GetBuilder<WalletController>(
                ...
                builder: (walletController) {
                  return walletController.isCoinPlanLoading
                      ...
                      : walletController.rewardCoins.isEmpty
                          ? Center(
                              child: NoDataWidget(text: EnumLocal.youDontHaveAnyRewardCoinsHistoryYet.name.tr)
                                  .paddingOnly(top: Get.height * .25),
                            )
                          : ListView.builder(
                              shrinkWrap: true,
                              physics: const NeverScrollableScrollPhysics(),
                              itemCount: walletController.rewardCoins.length,
                              itemBuilder: (context, index) {
                                final item = walletController.rewardCoins[index];
                                return Padding(
