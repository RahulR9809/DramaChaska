...
import 'package:drama_chaska/ui/login_page/api/login_api.dart';
...
onGuestLogin() async {
  try {
    ...
    // Log parameters
    log("loginType: ${Utils.getLoginTypeValue(LoginType.QUICK)}");
    log("email/identity: ${Preference.identity}");
    log("FCM Token: ${Preference.fcmToken}");
    ...
    print("Guest login API response: ${loginUserModel?.toJson()}");
    ...
    if (loginUserModel != null && loginUserModel?.status == true) {
      ...
      await Preference.onIsLogin(true);
      Get.offAllNamed(AppRoutes.bottomBarPage);
      log("Guest login successful, UserID: ${Preference.userId}");
    } else {
      log("Guest login failed: ${loginUserModel?.message}");
      Utils.showToast(Get.context!, loginUserModel?.message ?? "Guest login failed");
    }
  } catch (e) {
    log("Guest login exception: $e");
    Utils.showToast(Get.context!, "Error during guest login");
  } finally {
    if (Get.isDialogOpen ?? false) {
      Get.back();
    }
  ...
  //         await Preference.onIsLogin(true);
  //         Get.offAllNamed(AppRoutes.bottomBarPage);
  //         print("message UserID :: ${Preference.userId}");
  //       } else {
  //         Utils.showToast(Get.context!, loginUserModel?.message ?? "");
  //       }
  //     }
  ...
  //       print("loginUserModel :: ${loginUserModel.toJson()}");
  //     }
  //   } finally {
  //     if (Get.isDialogOpen ?? false) {
  //       Get.back();
